...
export const RetailApp = () => {
  ...
  const renderScreen = () => {
    ...
    if (showFavoritesScreen) {
      return <FavoritesScreen onBack={() => setShowFavoritesScreen(false)} products={products} onAddToCart={handleAddToCart} />;
    }
    if (showCustomScreen) {
    ...
    switch (activeTab) {
      case "order":
        return <NewOrderScreen onCustomClick={() => setShowCustomScreen(true)} onFavoritesClick={() => setShowFavoritesScreen(true)} products={products} onToggleFavorite={handleToggleFavorite} onAddToCart={handleAddToCart} />;
      case "transactions":
  ...
  return <RetailDevice>
      <div className="h-full flex flex-col overflow-hidden">
        ...
        {isLoggedIn && !showCustomScreen && !showFavoritesScreen && <BottomNavigation activeTab={activeTab} onTabChange={setActiveTab} />}
      </div>
    </RetailDevice>;
};