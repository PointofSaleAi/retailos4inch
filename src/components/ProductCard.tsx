...
export const ProductCard = ({ product, onAddToCart, hideImage = false }: ProductCardProps) => {
  ...
  return (
    <div className={`bg-surface rounded-lg overflow-hidden border border-border w-[90px] flex flex-col ${hideImage ? 'h-[50px]' : 'h-[102px]'}`}>
      {!hideImage && (
      ...
      <div className={`p-1 flex flex-col flex-1 ${hideImage ? 'flex-row items-center justify-between' : 'justify-between'}`}>
        {hideImage && (
          <Button
            variant="default"
            size="icon"
            onClick={() => handleQuantityChange(quantity + 1)}
            className="h-5 w-5 rounded-full bg-foreground text-background hover:bg-foreground/90 font-bold text-sm flex items-center justify-center p-0 flex-shrink-0"
          >
            +
          </Button>
        )}
        <div className={`flex flex-col ${hideImage ? 'flex-1 mx-1' : ''}`}>
        ...
        {hideImage && quantity > 0 && (
          <div className="bg-primary text-primary-foreground rounded-full w-4 h-4 flex items-center justify-center text-[8px] font-bold flex-shrink-0">
            {quantity}
          </div>
        )}
      </div>
